...
name: computer-use
description: >-
  Use Modular's computer-use CLI to inspect and operate local desktop app windows
  ...
  accessibility actions. Also use for browser windows, webviews, Modular app UI,
  or other desktop UI. Triggers include "computer use", "modular computer", "read
...
Use this skill for desktop UI through `modular computer`. When the requested target is a website or web app, operate the desktop browser app/window that contains the page.

## Preconditions

- Prefer `modular computer ...`; on Linux, use `modular-ide computer ...` if `modular` is unavailable. In this Modular worktree, use `./config/scripts/modular-dev computer ...` only when testing the local dev runtime.
...
```bash
modular status --json
modular computer capabilities --json
```

## Core Loop

```bash
modular computer list-apps --json
modular computer get-app-state --app com.spotify.client --json
modular computer click --app com.spotify.client --element-index 42 --json
```
...
```bash
modular computer get-app-state --app com.microsoft.edgemac --json
modular computer get-app-state --app Spotify --json
modular computer get-app-state --app pid:12345 --json
```
...
```bash
modular computer permissions --json
modular computer capabilities --json
modular computer list-apps --json
modular computer list-windows --app <app> --json
modular computer get-app-state --app <app> --json
modular computer get-app-state --app <app> --restore-window --json
modular computer click --app <app> --element-index <index> --json
modular computer click --app <app> --x 100 --y 100 --json
modular computer perform-secondary-action --app <app> --element-index <index> --action <name> --json
modular computer set-value --app <app> --element-index <index> --value "text" --json
modular computer type-text --app <app> --text "text" --json
modular computer press-key --app <app> --key Return --json
modular computer hotkey --app <app> --key CmdOrCtrl+A --json
modular computer paste-text --app <app> --text "text" --json
modular computer scroll --app <app> (--element-index <index> | --x <x> --y <y>) --direction down --json
modular computer drag --app <app> --from-element-index <index> --to-element-index <index> --json
modular computer drag --app <app> --from-x 100 --from-y 100 --to-x 300 --to-y 300 --json
```
...
```bash
printf '%s' "$TEXT" | modular computer set-value --app <app> --element-index <index> --value-stdin --json
```
...
```bash
modular computer get-app-state --app com.microsoft.edgemac --restore-window --json
modular computer set-value --app com.microsoft.edgemac --element-index <addressBarIndex> --value "test123" --json
modular computer press-key --app com.microsoft.edgemac --key Return --json
```
...
- `app_not_found`: run `list-apps` and retry with the bundle ID. If the target is a web app such as Gmail, choose the desktop browser app/window that contains it; do not retry `modular computer ... --app Gmail` unchanged because `modular computer` app selectors refer to desktop apps, not website names.
...
- `screenshot_failed`: use `--no-screenshot` if tree state is enough; if the message names Screen Recording or screenshots permission, run `modular computer permissions --id screenshots --json`.
- `accessibility_error`: run `modular computer capabilities --json`; if the message names Accessibility permission, run `modular computer permissions --id accessibility --json`.
- Empty tree or no screenshot: app may have no visible window, be minimized, or need permissions.
- Permission errors: run `modular computer permissions --json`, or `modular computer permissions --id accessibility --json` / `--id screenshots --json` when the message names one permission, use the setup UI, then retry.

## Next Action

Confirm Modular status unless already checked, then run `modular computer capabilities --json`. For website or web-app targets such as Gmail, identify the desktop browser app/window that contains the page, then get that target app state with `modular computer get-app-state --app <app> --json`.
